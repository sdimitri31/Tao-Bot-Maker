...
namespace BlueMystic
{
	...
	public class DarkModeCS
	{
        ...
        /// <summary>This tries to automatically apply Windows Dark Mode (if enabled) to a Form.</summary>
        /// <param name="_Form">The Form to become Dark</param>
        /// <param name="_Mode">'0' Enable Dark mode. '1' Enable Light mode. '2' Use Windows settings</param>
        /// <param name="_ColorizeIcons">[OPTIONAL] re-colorize all Icons in Toolbars and Menus.</param>
        /// <param name="_RoundedPanels">[OPTIONAL] make all Panels Borders Rounded</param>
        public DarkModeCS(Form _Form, int _Mode, bool _ColorizeIcons = true, bool _RoundedPanels = false)
        {
            //Sets the Properties:
            OwnerForm = _Form;
            ColorizeIcons = _ColorizeIcons;
        ...
        public void ThemeControl(Control control)
		{
			...
			if (control is FlatTabControl fTab)
			{
				...
				fTab.LineColor = Color.Transparent;
			}
		...
		public static OSThemeColors GetSystemColors(Form Window = null, bool IsDarkMode = false)
		{
            ...
            {
                _ret.Background = SystemColors.Control;   //<- Negro Claro
                ...
                //Apply Window's Light Mode to the Form's Title bar
                if (Window != null)
                {
                    ApplySystemDarkTheme(Window, false);
		...
		private static void ApplySystemDarkTheme(Control control = null, bool IsDarkTheme = true)
		{
			/* 			    
            ...
            if (IsDarkTheme)
				DarkModeOn = new[] { 0x01 }; //<- 1=True, 0=False
            else
                DarkModeOn = new[] { 0x00 }; //<- 1=True, 0=False
			...
			foreach (Control child in control.Controls)
			{
				if (child.Controls.Count != 0)
					ApplySystemDarkTheme(child, IsDarkTheme);
			}
        }       /// <summary>Attemps to apply Window's Dark Style to the Control and all its childs.</summary>
                /// <param name="control"></param>
	...
	public class MyRenderer : ToolStripProfessionalRenderer
	{
		...
		protected override void OnRenderItemImage(ToolStripItemImageRenderEventArgs e)
		{
			...
			else if (!ColorizeIcons && e.Image != null)
            {
                // Get the current icon
